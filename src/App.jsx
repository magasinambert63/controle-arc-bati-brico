...
const bad=s=>/(TOTAL|T\.?V\.?A|IBAN|SIRET|SIREN|APE|RCS|PAGE|ADRESSE|CONDITIONS|RÈGLEMENT|REGLEMENT|NET A PAYER|LIVRAISON|EMAIL|TÉL|TEL|FAX|BANQUE|TVA|MONTANT|ECO CONTRIBUTION|CODE DOUANIER|EAN|CLIENT|FOURNISSEUR|COMMANDE VENTE|CONFIRMATION DE COMMANDE)/i.test(s)

function dedupe(rows){
 const seen=new Set(),out=[]
 for(const r of rows){
  const k=(r.reference||'')+'|'+(r.quantite||'')+'|'+(r.designation||'').slice(0,22)
...
function extractCommandeBatiBrico(text){
 ...
 const rows=[]
 const re=/(\b[A-Z]?\d{5,12}[A-Z0-9]*\b)\s*\(Notre réf:?\s*([A-Z0-9]+)\)?\s+(.+?)\s+(\d+(?:[,.]\d+)?)\s*(PCE|PCS|ML|M2|M3|CAR|BTE|BT|ROU|SAC|UN|PAL)?\s+(\d+(?:[,.]\d+)?)(?:\s*(PCE|PCS|ML|M2|M3|CAR|BTE|BT|ROU|SAC|UN|PAL))?/gi
 let m
 while((m=re.exec(flat))!==null){
  const designation=clean(m[3]).replace(/Dont Eco.*$/i,'')
  if(!designation||bad(designation))continue
  rows.push({id:Date.now()+rows.length,reference:m[1],refBati:m[2],designation,quantite:fmt(qte(m[6]||m[4])),unite:m[7]||m[5]||'',prix:'',source:clean(m[0]),confiance:'Commande BATI'})
 }
 return dedupe(rows)
}
...
function extractArcUniversel(text){
 ...
 for(const line of lines){
  ...
  if(slash){qty=fmt(Math.max(...slash.split('/').map(nval)))}
   ...
   const simple=nums.find(x=>nval(x)>0&&nval(x)<10000)
  ...
  rows.push({id:Date.now()+rows.length,reference:ref,refBati:'',designation,quantite:qty,unite:'',prix:'',source:line,confiance:qty?'ARC':'Faible'})
 }
 if(rows.length<3){
  for(let i=0;i<lines.length;i++){
   ...
   if(r&&!bad(lines[i])){
    const ref=r[2]
    let designation='',qty=''
    for(let j=i+1;j<Math.min(i+4,lines.length);j++){if(!bad(lines[j])&&!/^EAN|Code douanier/i.test(lines[j])){designation=lines[j];break}}
    for(let j=i+1;j<Math.min(i+8,lines.length);j++){const q=lines[j].match(/(\d+(?:[,.]\d+)?)(?:\s*)?(PIL|PLA|PCE|PCS|ML|M2|M3|CAR|BTE|BT|ROU|SAC|SEA|UN|PAL)/i);if(q){qty=fmt(qte(q[1]));break}}
    if(designation)rows.push({id:Date.now()+rows.length,reference:ref,refBati:'',designation,quantite:qty,unite:'',prix:'',source:lines.slice(i,i+5).join(' '),confiance:qty?'ARC':'Faible'})
   }
  }
 }
 return dedupe(rows)
...
function compareRows(cmd,arc){
 ...
 for(const c of cmd){
  const idx=arc.findIndex((a,i)=>!used.has(i)&&a.reference&&c.reference&&a.reference.toLowerCase()===c.reference.toLowerCase())
  ...
  const ok=!cq||!aq||Math.abs(cq-aq)<0.0001
 ...
 arc.forEach((a,i)=>{if(!used.has(i)&&a.reference)out.push({status:'SUPPLEMENTAIRE',reference:a.reference,refBati:'',designation:a.designation,message:'Article présent sur ARC mais absent de la commande',cmd:null,arc:a})})
 return out
}
...
export default function App(){
 ...
 const importFile=async(file,target)=>{
  ...
  try{
   ...
   if(file.type==='application/pdf'||file.name.toLowerCase().endsWith('.pdf')){
    ...
    if(!text||text.length<50){setStatus('PDF scanné détecté : OCR automatique en cours...');text=await ocrFile(file,setStatus)}
    else setStatus('Texte PDF extrait automatiquement.')
   }else if(file.type.startsWith('image/')){
    text=await ocrFile(file,setStatus);setStatus('OCR image terminé.')
 ...
 const exportCSV=kind=>{
  ...
  else{const s=kind==='cmd'?cmdRows:arcRows;rows=[['Réf fournisseur','Réf BATI','Désignation','Quantité','Unité','Source'],...s.map(r=>[r.reference,r.refBati,r.designation,r.quantite,r.unite,r.source])];fn=kind==='cmd'?'commande_bati_brico_normalisee.csv':'arc_fournisseur_normalise.csv'}
 ...
 return <div className="app">
  <header><div><h1>BATI BRICO</h1><p>Contrôle ARC fournisseur V2.2 — commande BATI BRICO + ARC universel</p></div><div className="status"><span>Contrôle</span><b>{results.length?`${ok} OK / ${ec+man+sup} alerte(s)`:'En attente'}</b></div></header>
